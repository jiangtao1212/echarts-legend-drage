      ...
      <VueDraggable v-for="(data, index) in dataAbout.list" :key="data.key" v-show="data.value.length > 0"
        ...
        <div v-for="item in dataAbout.list[index].value" :key="item.id"
          ...
          <el-popover placement="right" :width="100"
            ...
            trigger="contextmenu">
            <template #reference>
              <span class="cursor-move-item" :style="{ 'color': colors[(+item.id - 1) % colors.length] }">{{ item.name}}</span>
            </template>
...
clickObjFun.deleteItemDefault = function (itemId: string) {
  ...
  // 1.删除当前子项
  dataOrigin.forEach((item: listDataType) => item.value = item.value.filter((value: valueType) => value.id !== itemId));
  // 2.调整各个子项id，大于当前子项的id减1，使其保持连续
  dataOrigin.forEach((item: listDataType) => {
    item.value.forEach((value: valueType, index: number) => {
      if (value.id > itemId) {
        value.id = (+value.id - 1).toString();
      }
    });
  });
  const count = dataOrigin.reduce((acc: number, cur: listDataType) => acc + cur.value.length, 0);
  // 3.新建空列表，填充对应数据
  const resetData: Array<listDataType> = new Array(count);
  for (let i = 0; i < count; i++) {
    resetData[i] = JSON.parse(JSON.stringify({ key: (i + 1).toString(), value: [] }));
  }
  resetData.forEach((item1: listDataType) => {
    const key = item1.key;
    dataOrigin.forEach((item2: listDataType, index: number) => {
